...
class Sage182Spider(scrapy.Spider):
    name = 'SAGE182'
    start_urls = ['http://journals.sagepub.com/toc/idra/11']
    ...
    def parse2(self, response):
        ...
        item['DOI'] = response.xpath("//div[@class='widget-body body body-none  body-compact-all']/div[@class='doiWidgetContainer']/a[@class='doiWidgetLink']/text()").extract()[
            0]
        #         # print(item['DOI'])
        #         # 没有关键词
        ...
        if response.xpath("//div[@class='rightMobileMenuButton articleToolsButton PDFTool pdf-access redButton smallButton']/a/@href").extract():
            pdf = response.xpath("//div[@class='rightMobileMenuButton articleToolsButton PDFTool pdf-access redButton smallButton']/a/@href").extract()[
                0]
